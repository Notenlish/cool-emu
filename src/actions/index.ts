...
import { formSchema, feedbackSchema } from '$lib/data/schema';
...
export const server = {
        ...
        handler: async (input) => {
            try {
                ...
                if (error instanceof ZodError) {
                    ...
                    throw new ActionError({ code: "BAD_REQUEST", message: `Error! ${error}` })
                }
            }
    ...
    sendFeedback: defineAction({
        accept: "form", input: feedbackSchema, handler: async (input) => {
            try {
            ...
            if (!response.ok) throw new ActionError({
                code: "INTERNAL_SERVER_ERROR", message: "Server had an issue processing your request."
            });
            return "Successfully sent feedback."
        }
    })
}